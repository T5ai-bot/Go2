        ...
        121645100408832000.0, 2432902008176640000.0
                          ...
                          float   stance_phase_signal)
            {
                ...
                prev_foot_position_ = foot_position;
                    ...
                    void fitTemplate(const std::vector<float>& x,
                         const std::vector<float>& y);

    };
 /* ─────────────  把任意长度模板拉伸/压缩 → 20 点 ───────────── */
void TrajectoryPlanner::fitTemplate(const std::vector<float>& tx,
                                    const std::vector<float>& ty)
{
    assert(tx.size() == ty.size() && tx.size() >= 2);

    const size_t N = total_control_points_;   // =20
